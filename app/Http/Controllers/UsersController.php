...
class UsersController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('profile')) {
            $imagePath = $request->file('profile')->store('public/users-profile');
        }
        User::create([
            'name' => $request->name,
    ...
    public function update(Request $request, $username)
    {
        ...
        if($request->hasFile('profile')){
            if($request->oldProfile){
                Storage::delete('public/users-profile/' . $request->oldProfile); 
            }
            $imagePath = $request->file('profile')->store('public/users-profile');
            $user->profile = basename($imagePath);
        }else{    
            $user->profile = $request->oldProfile;
        }
    ...
    {
        $user = User::where('username', $username)->firstOrFail();
        Storage::delete('public/users-profile/' . $user->profile);
        $user->delete();
