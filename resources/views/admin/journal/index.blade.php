        ...
        <table class="w-full table-auto">
            <thead class="bg-gray-200 text-gray-700">
                <tr>
                    ...
                    <th class="p-4 text-left">PDF URL</th> 
...
<!-- PDF Modal -->
<div id="pdfModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto mb-10 h-full w-full hidden">
...
<script>
    ...
    function renderTable() {
        ...
        const filteredJournals = journals.slice(startIndex, currentPage * itemsPerPage);
        filteredJournals.forEach((journal, index) => {
            const row = `<tr class="border-b">
                ...
                <td class="p-4"><a href="${journal.pdf}" target="_blank" class="text-blue-500 hover:underline">Download PDF</a></td> <!-- Tampilkan URL PDF -->
                <td class="p-4"><a class="hover:bg-slate-300 rounded-lg px-3 py-2" href="#" onclick="openPdfModal('${journal.pdf}', '${journal.judul}'); return false;"><i class="fa-regular fa-eye"></i></a></td> <!-- Preview PDF -->
    ...
    function filterJournals() {
        ...
        filtered.slice(0, itemsPerPage).forEach((journal, index) => {
            const row = `<tr class="border-b">
                ...
                <td class="p-4">${journal.tahun}</td>
                <td class="p-4">${journal.sumber}</td>
                <td class="p-4"><a href="${journal.pdf}" download="${journal.pdf}" class="text-blue-500 hover:underline">Download PDF</a></td>
                <td class="p-4"><a class="hover:bg-slate-300 rounded-lg px-3 py-2" href="#" onclick="openPdfModal('${journal.pdf}', '${journal.judul}'); return false;"><i class="fa-regular fa-eye"></i></a></td>
                <td class="p-4 flex mt-2.5 gap-2.5">
                    <a href="/dashboard/journals/${journal.slug}/edit" class="block text-blue-500 hover:underline"><i class="fa-regular fa-pen-to-square"></i></a>
                    <form action="/dashboard/journals/${journal.slug}" method="POST" onsubmit="return confirm('Are you sure you want to delete this item?');">
                        @csrf
