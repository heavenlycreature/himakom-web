...
<div class="flex lg:bg-gray-100 rounded-xl">
    <form class="w-full max-w-full mx-auto p-6 rounded-xl shadow-md" method="POST" action="{{ route('users.store') }}" enctype="multipart/form-data">
        ...
        <div class="mt-6">
            <label for="role" class="block mb-2 text-sm font-medium @error('role') text-red-700 dark:text-red-500 @else text-gray-700 @enderror">
                Role
            </label>
            <select 
                id="role" 
                name="role" 
                class="text-sm rounded-lg block w-full p-2.5 @error('role') bg-red-50 border border-red-500 text-red-900 placeholder-red-700 focus:ring-red-500 focus:border-red-500 dark:bg-gray-700 dark:text-red-500 dark:placeholder-red-500 dark:border-red-500 @else bg-gray-50 border border-gray-300 text-gray-900 focus:ring-blue-500 focus:border-blue-500 @enderror"  
                required>
